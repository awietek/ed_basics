...
# Step 1: just copied this function from hamiltonian_hb_xxz.py
def get_hamiltonian_sparse(L, J, delta, sz, k):
    '''
    Creates the Hamiltonian of the Heisenberg XXZ model 
    '''
...
# Step 2: evaluate lowest 10 eigenvalues for each value of k
L=12
J=1
delta=0
sz=0
...
for k in range(L+1):
    ...
    eigs = sp.sparse.linalg.eigsh(hamiltonian, k=n_lowest_eigenvalues,
                                  ...
                                  maxiter=1000)
    print(k, np.sort(eigs))
    plt.plot(k*2*np.pi/L*np.ones_like(eigs), eigs, 's',
             color="steelblue")
